...
public final class TensorBuilder
{
    ...
    /**
     * Creates {@link Tensor} instance with the same size and information as the given {@link INDArray}.
     * 
     * @param sequence
     ...
     *         If the type of the sequence is not supported.
     */
    public static Tensor<? extends TType> build(INDArray array) throws IllegalArgumentException
    {
    ...
    /**
     * Creates a {@link Tensor} of type {@link TUint8} from an {@link INDArray} of type {@link DataType#BYTE} 
     ...
     */
    private static Tensor<TUint8> buildByte(INDArray array) throws IllegalArgumentException
    {
        if (array.dataType() != DataType.INT8 && array.dataType() != DataType.UINT8)
		...
		Tensor<TUint8> tensor = Tensor.of(TUint8.DTYPE, Shape.of(array.shape()), dataBuffer);
		return tensor;
    }

    /**
     * Creates a {@link Tensor} of type {@link TInt32} from an {@link INDArray} of type {@link DataType#INT} or
     * {@link DataType#UINT}.
     * 
     * @param sequence
     *        The sequence to fill the tensor with.
     ...
     *         If the type of the image is not compatible.
     */
    private static Tensor<TInt32> buildInt(INDArray array) throws IllegalArgumentException
    {
		...
		Tensor<TInt32> tensor = TInt32.tensorOf(Shape.of(array.shape()), dataBuffer);
		return tensor;
    }
     ...
     *         If the type of the image is not compatible.
     */
    private static Tensor<TInt64> buildLong(INDArray array) throws IllegalArgumentException
    {
        if (array.dataType() != DataType.INT32 && array.dataType() != DataType.UINT32)
            throw new IllegalArgumentException("Image is not of int type: " + array.dataType());

        LongDataBuffer dataBuffer = RawDataBufferFactory.create(array.data().asLong(), false);
		Tensor<TInt64> tensor = TInt64.tensorOf(Shape.of(array.shape()), dataBuffer);
		return tensor;
    }

    /**
     * Creates a {@link Tensor} of type {@link TFloat32} from an {@link INDArray} of type {@link DataType#FLOAT}.
     * 
     * @param array
     *        The sequence to fill the tensor with.
     * @return The tensor filled with the image data.
     ...
     *         If the type of the image is not compatible.
     */
    private static Tensor<TFloat32> buildFloat(INDArray array) throws IllegalArgumentException
    {
		...
		Tensor<TFloat32> tensor = TFloat32.tensorOf(Shape.of(array.shape()), dataBuffer);
		return tensor;
    }
     ...
     *         If the type of the image is not compatible.
     */
    private static Tensor<TFloat64> buildDouble(INDArray array) throws IllegalArgumentException
    {
        if (array.dataType() != DataType.DOUBLE)
            throw new IllegalArgumentException("Tensor is not of float type: " + array.dataType());

        DoubleDataBuffer dataBuffer = RawDataBufferFactory.create(array.data().asDouble(), false);
		Tensor<TFloat64> tensor = TFloat64.tensorOf(Shape.of(array.shape()), dataBuffer);
		return tensor;
    }
     ...
     *         If the type of the image is not compatible.
     */
    private static Tensor<TBool> buildBoolean(INDArray array) throws IllegalArgumentException
    {
        if (array.dataType() != DataType.BOOL)
            throw new IllegalArgumentException("Tensor is not of boolean type: " + array.dataType());

        ByteDataBuffer dataBuffer = RawDataBufferFactory.create(array.data().asBytes(), false);
		Tensor<TBool> tensor = Tensor.of(TBool.DTYPE, Shape.of(array.shape()), dataBuffer);
		return tensor;
    }
}
